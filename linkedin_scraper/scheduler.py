...
def is_within_operating_hours(current_time: datetime = None) -> Tuple[bool, str]:
    ...
    if current_minutes < start_minutes:
        return False, f"Before operating hours. Current time: {current_time.strftime('%H:%M')}, Operating hours: {config.OPERATING_START_HOUR:02d}:{config.OPERATING_START_MINUTE:02d}-{config.OPERATING_END_HOUR:02d}:{config.OPERATING_END_MINUTE:02d}"
    
    if current_minutes > end_minutes:
        return False, f"After operating hours. Current time: {current_time.strftime('%H:%M')}, Operating hours: {config.OPERATING_START_HOUR:02d}:{config.OPERATING_START_MINUTE:02d}-{config.OPERATING_END_HOUR:02d}:{config.OPERATING_END_MINUTE:02d}"
