"""LinkedIn Sales Navigator - Accurate Profile Card Scraper (tab-switch fix)"""
import pyautogui
import time
import json
import random
...
# Safe focus point (blank page area) before hotkeys
SAFE_FOCUS_X = 800
SAFE_FOCUS_Y = 200
...
def get_profile_card_coordinates(profile_num):
    """Return coordinates for clicking on profile cards"""
    profile_positions = [
        (420, 365),
        (420, 515),
        (420, 692),
...
def focus_browser_body():
    """Click a safe spot so browser receives keyboard shortcuts consistently."""
    try:
...
def is_profile_view_heuristic():
    """
    OCR heuristic:
      profile page likely includes: about/activity/experience/featured
      list page likely includes: search/filters/results/lead filters
    """
    if not OCR_AVAILABLE:
        return True  # can't verify, assume okay

    lines = extract_full_screen()
    blob = " ".join(lines).lower()

    profile_hits = sum(k in blob for k in ["about", "activity", "experience", "featured"])
    list_hits = sum(k in blob for k in ["search", "filters", "results", "lead filters"])

    return profile_hits >= 1 and profile_hits >= list_hits


def switch_to_new_profile_tab(wait_timeout=12):
    """
    Robustly switch to the newly opened tab and confirm we are on profile page.
    """
    attempts = 3
    for attempt in range(1, attempts + 1):
        ...
        if not OCR_AVAILABLE:
            logger.info(f"Switched tabs (OCR unavailable, trusting attempt {attempt})")
            return True

        start = time.time()
        while time.time() - start < wait_timeout:
            if is_profile_view_heuristic():
                logger.info(f"Switched to profile tab (attempt {attempt})")
                return True
            time.sleep(0.7)

        logger.warning(f"Tab switch not confirmed (attempt {attempt}), retrying...")

    return False


def close_current_tab_and_return():
    focus_browser_body()
...
def scrape_profile_with_screenshot(profile_num):
    ...
    try:
        ...
        logger.info("Step 4: Waiting for new tab spawn...")
        time.sleep(random.uniform(2.5, 4.0))

        logger.info("Step 5: Switching to new tab (robust)...")
        if not switch_to_new_profile_tab(wait_timeout=12):
            logger.error("Could not confirm profile tab switch.")
            return None
        ...
        profile_data = {
            ...
            "new_tab_button": {"x": NEW_TAB_BUTTON_X, "y": NEW_TAB_BUTTON_Y},
            "text": profile_text,
...
def save_data(profiles):
    ...
    try:
        ...
        with open(txt_file, 'w', encoding='utf-8') as f:
            f.write("LinkedIn Profiles - Slow Scroll Extraction\n")
            ...
            for p in profiles:
                ...
                f.write(f"New Tab Btn: {p.get('new_tab_button')}\n")
